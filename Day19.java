...
/* Extra Materials:
        #Interface: An interface is a collection of abstract methods and constants that form a common set of base
                    ...
                    separate standalone classes), we have a guarantee that all three classes (and any future classes that
                    implement Polygon) will follow the same basic rules. Knowing the rules for what any class implementing
                    Polygon will do enables us independently write code that uses some type of Polygon, regardless of how
                    it's implemented.
                    Another benefit is that if we decide to improve upon our Polygon interface by adding a double getArea()
                    method, our code will not compile unless we add an implementation for getArea to each class that
                    implements Polygon.
*/